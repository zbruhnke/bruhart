          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid gap-8">
                    ...
                    <div className="divide-y divide-border">
                      {page.rfqChecklist.map((item) => (
                        <div key={item.label} className="grid gap-2 py-4 sm:grid-cols-[150px_1fr]">
                          <dt className="text-sm font-semibold uppercase tracking-wider text-foreground">{item.label}</dt>
                          <dd className="text-sm leading-6 text-foreground-muted">{item.detail}</dd>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="rounded-lg border border-border bg-white p-6">
                    ...
                    <div className="divide-y divide-border">
                      {page.availabilityNotes.map((item) => (
                        <div key={item.label} className="grid gap-2 py-4 sm:grid-cols-[150px_1fr]">
                          <dt className="text-sm font-semibold uppercase tracking-wider text-foreground">{item.label}</dt>
                          <dd className="text-sm leading-6 text-foreground-muted">{item.value}</dd>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="grid gap-8 lg:grid-cols-3">
                ...
                {page.proofAssets && page.proofAssets.length > 0 && (
                  <div className="rounded-lg border border-border bg-white p-6">
                    <h2 className="mb-5 text-xl font-bold text-foreground">Proof Assets</h2>
                    <div className="space-y-4">
                      {page.proofAssets.map((item) => (
                        <article key={item.label} className="rounded-lg bg-background-alt p-4">
                          <h3 className="mb-2 font-semibold text-foreground">
                            {item.href ? (
                              <Link href={item.href} className="text-primary hover:text-primary-dark">
